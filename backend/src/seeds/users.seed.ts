import * as bcrypt from 'bcryptjs';

export interface User {
  id: string;
  name: string;
  email: string;
  passwordHash: string;
  roleId: string;
  roleName: string;
  phone: string;
  designation: string;
  isActive: boolean;
  managerId: string | null;
  teamLeadId: string | null;
  createdAt: string;
  avatar?: string;
}

const hash = (pwd: string) => bcrypt.hashSync(pwd, 10);

export const USERS_SEED: User[] = [
  {
    id: 'usr_admin',
    name: 'Admin User',
    email: 'admin@zsmart.com',
    passwordHash: hash('Admin@123'),
    roleId: 'role_admin',
    roleName: 'admin',
    phone: '9000000001',
    designation: 'System Administrator',
    isActive: true,
    managerId: null,
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_manager',
    name: 'Ravi Kumar',
    email: 'manager@zsmart.com',
    passwordHash: hash('Manager@123'),
    roleId: 'role_manager',
    roleName: 'manager',
    phone: '9000000002',
    designation: 'Regional Manager',
    isActive: true,
    managerId: null,
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_teamlead',
    name: 'Priya Singh',
    email: 'teamlead@zsmart.com',
    passwordHash: hash('Lead@123'),
    roleId: 'role_team_lead',
    roleName: 'team_lead',
    phone: '9000000003',
    designation: 'Team Lead',
    isActive: true,
    managerId: 'usr_manager',
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_exec',
    name: 'Arjun Das',
    email: 'exec@zsmart.com',
    passwordHash: hash('Exec@123'),
    roleId: 'role_field_executive',
    roleName: 'field_executive',
    phone: '9000000004',
    designation: 'Field Executive',
    isActive: true,
    managerId: 'usr_manager',
    teamLeadId: 'usr_teamlead',
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_finance',
    name: 'Meena Sharma',
    email: 'finance@zsmart.com',
    passwordHash: hash('Finance@123'),
    roleId: 'role_finance_user',
    roleName: 'finance_user',
    phone: '9000000005',
    designation: 'Finance Executive',
    isActive: true,
    managerId: null,
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  // ── Pre-Sales Users ─────────────────────────────────────────────
  {
    id: 'usr_sales',
    name: 'Vikram Nair',
    email: 'sales@zsmart.com',
    passwordHash: hash('Sales@123'),
    roleId: 'role_sales_executive',
    roleName: 'sales_executive',
    phone: '9000000006',
    designation: 'Senior Sales Executive',
    isActive: true,
    managerId: 'usr_manager',
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_engineer',
    name: 'Deepak Sharma',
    email: 'engineer@zsmart.com',
    passwordHash: hash('Engg@123'),
    roleId: 'role_engineering_user',
    roleName: 'engineering_user',
    phone: '9000000007',
    designation: 'Solar Design Engineer',
    isActive: true,
    managerId: 'usr_manager',
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
  {
    id: 'usr_commercial',
    name: 'Sunita Patel',
    email: 'commercial@zsmart.com',
    passwordHash: hash('Comm@123'),
    roleId: 'role_commercial_user',
    roleName: 'commercial_user',
    phone: '9000000008',
    designation: 'Commercial Manager',
    isActive: true,
    managerId: 'usr_manager',
    teamLeadId: null,
    createdAt: '2024-01-01T00:00:00Z',
  },
];
